...
# =============================================================================
# SPONSOR FEEDBACK (loaded from data/sponsor_feedback.json)
# =============================================================================

_SPONSOR_CACHE: Optional[Dict[str, Dict]] = None

def _load_sponsor_feedback() -> Dict[str, Dict]:
    """Load sponsor feedback from data/sponsor_feedback.json once and cache."""
    global _SPONSOR_CACHE
    if _SPONSOR_CACHE is not None:
        return _SPONSOR_CACHE
    base_dir = os.path.dirname(os.path.abspath(__file__))
    data_path = os.path.join(base_dir, 'data', 'sponsor_feedback.json')
    if not os.path.exists(data_path):
        data_path = os.path.join(os.path.dirname(base_dir), 'data', 'sponsor_feedback.json')
    try:
        with open(data_path, 'r', encoding='utf-8') as f:
            data = json.load(f)
        _SPONSOR_CACHE = {str(k).strip().lower(): v for k, v in data.items()}
        logger.info(f"Loaded {len(_SPONSOR_CACHE)} sponsor entries from {data_path}")
    except Exception as e:
        logger.warning(f"Could not load sponsor feedback from {data_path}: {e}")
        _SPONSOR_CACHE = {}
    return _SPONSOR_CACHE


def get_sponsor_for_name(name: Any) -> Dict:
    """Return sponsor info dict for a candidate, or empty dict if not found."""
    sponsors = _load_sponsor_feedback()
    if not sponsors:
        return {}
    key = normalize_name(name)
    if key in sponsors:
        return sponsors[key]
    variant = key.replace('isaac ', 'issac ').replace('issac ', 'isaac ')
    if variant in sponsors:
        return sponsors[variant]
    for sp_key, sp_val in sponsors.items():
        if fuzzy_match_name(sp_key, key, threshold=0.82):
            return sp_val
    return {}
    ...
    try:
        ...
        for placed_mit in placed_mits:
            ...
            alumni_profile = {
                ...
                'sponsor': get_sponsor_for_name(name)
            }
